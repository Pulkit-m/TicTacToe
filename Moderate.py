...
Entering any other format will end up the game
...
def auto():
    ...
    if switch==0:
        hit_n_trial=0
        while hit_n_trial<8:
           trick=[[0,0],[1,1],[2,2],[2,0],[0,2]]    #these are not to be chosen in first move
           [i,j]=random.choice(trick)               #as this choice will always defeat it
           if a[i][j]=='.':
              a[i][j]=cpu
              break
           hit_n_trial+=1
...
while True:
  ...
  while True:
     ...
     if flag==1:
         ...
         break
     #next move is of a choice whether player 2 or cpu:
